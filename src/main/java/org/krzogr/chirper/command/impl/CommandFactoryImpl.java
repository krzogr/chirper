...
public final class CommandFactoryImpl implements CommandFactory {
	...
	public CommandFactoryImpl(final UserManager userManager, final PrintStream output, final Clock clock) {
	...
	public Runnable createAddPostCommand(final String userName,final String postText) {
		return new AddPostCommand(userManager, userName, postText, LocalDateTime.now(clock));
	}
